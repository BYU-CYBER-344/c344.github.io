...
In this class we will create new containers, not just use existing ones.
...
Be sure to meet all writeup requirements.

### Requirements for Hypervisors

If you choose one of the preferred options, *and your host system meets minimum requirements*, then you can be confident that your solution will meet these requirements.
...
* System must be capable of allocating at least 4GB to a running VM. (No need to run more than one VM at a time.)

### Requirements for Container Hosts

If your system could handle the containers from CYBER 210 then it will be sufficient for this class.

* Must be capable of hosting OCI (Open Container Initiative) format containers. (This is the format used by Docker.)
* System must be capable of allocating at least 1GB of memory and 3GB of storage to a container.
...
3. Following the instructions for your hypervisor, install Ubuntu Desktop as the guest OS in a VM.
4. Ensure your copy of Ubuntu works and that it has access to the internet. Use at least one GUI and one CLI app. Here are some good options.
    * **GUI** Open the bundled Firefox browser and browse to a few websites.
    * **CLI** From the terminal command line use some network-related commands such as `ifconfig`, `ping`, `curl`, etc. As needed, use `apt install` to install needed tools.
...
* [9 Points] Your choice of hypervisor:
    ...
    * How satisfied are you with your choice of hypervisor?
    * Other comments (optional).
* [5 Points] Two hypervisor screenshots.
...
* [9 Points] Your choice of container host:
    ...
    * How satisfied are you with your choice of container host?
    * Other comments (optional).
...
The following should be sufficient information to get you started with each hypervisor option. Look at the documentation for each hypervisor, check with the TAs, or ask your favorite AI for additional information. *But don't turn over the task to an AI!*

### VMWare running on a lab computer.
This is easiest and quickest.
However, it requires you to be in the lab whenever you are working with your VMs -- something you will be doing throughout the semester.
You may choose to put your VM configuration and disk image on a (large, fast) thumb drive so that you are not tied to one particular computer.
...
This comes with a significant performance penalty, but you may find that the convenience of running on your own computer is worth it.
For this exercise, you can install the ARM-based Ubuntu distribution on UTM in which case it will run at full CPU speed.

*These instructions are preliminary until a mac-running student or TA fills in details.*
...
There are fewer options for container hosts. And if you still have your Docker Desktop setup from CYBER 210 you can just use that.
